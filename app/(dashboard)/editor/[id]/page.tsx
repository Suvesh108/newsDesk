...
const mockArticle = {
  ...
  content: '<h2>The Turning Point</h2><p>The transition from narrow AI to something resembling general intelligence didn\'t arrive with a dramatic announcement...</p>',
  ...
  metaDesc: 'Major central banks announce synchronized adjustment to interest rate targets.',
}
...
export default function EditArticlePage() {
  ...
  const [category, setCategory] = useState<Category>(mockArticle.category)
  const [tags, setTags] = useState<string[]>(mockArticle.tags)
  const [coverImage, setCoverImage] = useState(mockArticle.coverImage)
  const [excerpt, setExcerpt] = useState(mockArticle.excerpt)
  const [metaTitle, setMetaTitle] = useState(mockArticle.metaTitle)
  const [metaDesc, setMetaDesc] = useState(mockArticle.metaDesc)
    ...
    <div className="min-h-screen">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-10">
        <div className="flex items-center gap-3 lg:gap-4">
          ...
          <h1 className="text-2xl lg:text-4xl font-black">Edit Article</h1>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
